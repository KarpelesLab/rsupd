...
fn setup_github_ci(project_dir: &Path, bins: &[String]) -> rsupd::Result<()> {
    ...
    let content = GITHUB_BUILD_YML.replace("__ARTIFACT_PATHS__\n", &paths);
...
/// replaced with the project's per-bin artifact paths.
...
# that triple — the layout `rsupd publish --ci` consumes.
